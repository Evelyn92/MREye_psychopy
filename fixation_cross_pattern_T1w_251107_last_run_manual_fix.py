...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    dot_2.extend([
        visual.Circle(
            ...
            radius=0.3,
            lineWidth=0.1,
            fillColor=[-1.0000, -1.0000, 0.0902],
            interpolate=True,
        ),
        visual.ShapeStim(
            win=win,
    ...
    dot_3.extend([
        visual.Circle(
            ...
            radius=0.3,
            lineWidth=0.1,
            fillColor=[1.0000, 0.0745, -0.4667],
            interpolate=True,
        ),
        visual.ShapeStim(
            win=win,
    ...
    dot_5.extend([
        visual.Circle(
            ...
            radius=0.3,
            lineWidth=0.1,
            fillColor=[0.4510, 0.4275, 0.4353],
            interpolate=True,
        ),
        visual.ShapeStim(
            win=win,
    ...
    dot_6.extend([
        visual.Circle(
            ...
            radius=0.3,
            lineWidth=0.1,
            fillColor=[1.0000, 0.7647, -0.6235],
            interpolate=True,
        ),
        visual.ShapeStim(
            win=win,
    ...
    dot_8.extend([
        visual.Circle(
            ...
            radius=0.3,
            lineWidth=0.1,
            fillColor=[0.3412, -0.1294, -0.5059],
            interpolate=True,
        ),
        visual.ShapeStim(
            win=win,
    ...
    dot_9.extend([
        visual.Circle(
            ...
            radius=0.3,
            lineWidth=0.1,
            fillColor=[0.6078, -0.2784, -0.2784],
            interpolate=True,
        ),
        visual.ShapeStim(
            win=win,
    ...
    dot_10.extend([
        visual.Circle(
            ...
            radius=0.3,
            lineWidth=0.1,
            fillColor=[-0.0039, 1.0000, 0.6627],
            interpolate=True,
        ),
        visual.ShapeStim(
            win=win,
    ...
    for thisT1w_LIBRE in T1w_LIBRE:
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # if component_cross is starting this frame...
            for component_cross in dot_2:
                if component_cross.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
            ...
            for component_cross in dot_3:
                if component_cross.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
            ...
            for component_cross in dot_4:
                if component_cross.status == NOT_STARTED and tThisFlip >= 3-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
            ...
            for component_cross in dot_5:
                if component_cross.status == NOT_STARTED and tThisFlip >= 4-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
            ...
            # *dot_6* updates
            for component_cross in dot_6:
            # if dot_6 is starting this frame...
                if component_cross.status == NOT_STARTED and tThisFlip >= 5-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
            ...
            # *dot_7* updates
            for component_cross in dot_7:
                # if dot_7 is starting this frame...
                if component_cross.status == NOT_STARTED and tThisFlip >= 6-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
            ...
            # *dot_8* updates
            for component_cross in dot_8:
                # if dot_8 is starting this frame...
                if component_cross.status == NOT_STARTED and tThisFlip >= 7-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
            ...
            # *dot_9* updates
            for component_cross in dot_9:
                # if dot_9 is starting this frame...
                if component_cross.status == NOT_STARTED and tThisFlip >= 8-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
            ...
            # *dot_10* updates
            for component_cross in dot_10:
                # if dot_10 is starting this frame...
                if component_cross.status == NOT_STARTED and tThisFlip >= 9-frameTolerance:
                    # keep track of start time/frame for later
                    component_cross.frameNStart = frameN  # exact frame index
                    component_cross.tStart = t  # local t and not account for scr refresh
                    component_cross.tStartRefresh = tThisFlipGlobal  # on global time
