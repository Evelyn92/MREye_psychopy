...
expName = 'fixation_dots_T2weighted'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Projects\\checkerboard_retina_fMRI\\fixation_dots_T2weighted_lastrun.py',
...
ioConfig['eyetracker.hw.sr_research.eyelink.EyeTracker'] = {
    ...
    'default_native_data_file_name': 'JB2',
    'runtime_settings': {
...
T1w_LIBRE = data.TrialHandler(nReps=1105.0, method='random', 
...
for thisT1w_LIBRE in T1w_LIBRE:
    ...
    if routineForceEnded:
        ioServer.getDevice('tracker').sendMessage("Routine T2w ended")
        routineTimer.reset()
    else:
        routineTimer.addTime(-1.000000)
    thisExp.nextEntry()
    
# completed 1105.0 repeats of 'T1w_LIBRE'
...
while continueRoutine:
    ...
    # *ET_stop* updates
    if ET_stop.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'ET_stop.started')
